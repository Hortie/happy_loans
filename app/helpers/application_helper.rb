...
def capital_restant_du(loan, ddate)
  ss = Schedule.where(loan_id: loan.id, tokeep: true).where("due_date > ?", ddate).order(:due_date).first
...
def capital_restant_du_total(ddate)
  ...
  sum= 0
  loan_ids.each do |id|
    ss = Schedule.where(loan_id: id, tokeep: true).where("due_date > ?", ddate).order(:due_date).first
...
def last_due_date(loan, ddate)
  ss = Schedule.where(loan_id: loan.id, tokeep: true).order(:due_date).last
